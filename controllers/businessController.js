  ...
  for (const file of files[field]) {
    try {
      ...
      if (s3Url) {
        ...
        console.warn(`⚠️ Upload failed for file ${file.originalname || 'unknown'} in field "${field}"`);
      }
    } catch (err) {
      console.warn(`❌ Error uploading file "${file.originalname || 'unknown'}" in field "${field}":`, err.message);
    }
  }
}
